...
function oer_course_create_edit_page_after_load_scripts() {
    //Angular and Custom JS Script Loaded
    wp_register_script('ng-ctrlr', plugins_url('/core/oer/assets/js/angular_controllers_course_oer.js?v=1', __DIR__));
...
function oer_course_create_edit_page_init() {
    require_once realpath(__DIR__ . '/../') . '/core/modules/resource-course/create/functions.php';
...
function register_elementor_oer_course_course_create_edit_page_widget($widgets_manager) {
    require_once(__DIR__ . '/CurOerCourseCreateEditPageWidget.class.php');
    $widgets_manager->register( new \CurOerCourseCreateEditPageWidget() );
}
add_action('elementor/widgets/register', 'register_elementor_oer_course_course_create_edit_page_widget');

function oer_course_create_edit_page_after_render( $element ) {
    if (is_object($element) && get_class($element) === 'CurOerCourseCreateEditPageWidget') {
        oer_course_create_edit_page_after_load_scripts();
    }
}
add_action( 'elementor/frontend/after_render', 'oer_course_create_edit_page_after_render' );


function oer_course_create_edit_page_before_render( $element ) {
    if (is_object($element) && get_class($element) === 'CurOerCourseCreateEditPageWidget') {
        oer_course_create_edit_page_init();
    }
}
add_action( 'elementor/frontend/before_render', 'oer_course_create_edit_page_before_render' );


/*
function oer_course_page_widget_after_display( $element ) {
...
add_action( 'elementor/frontend/after_render', 'oer_course_page_widget_after_display' );
*/
