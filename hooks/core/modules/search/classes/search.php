...
class search {
    ...
    public $search_results_header_data = array();
    public $search_input_widget_data = array();
    public $search_pagination_data = array();
    public $search_result_resources_data = array();
    public $query = "";
        ...
        if(isset($output) && is_array($output)){
            foreach ($output as $ind => $row) {
                ...
                if (isset($output[$ind]['fields']['license'])) {
                    ...
                    $output[$ind]['fields']['displayname'] = $this->wpdb->get_var("select displayname FROM licenses WHERE name = '" . $row['fields']['license'] . "'");
                }
    ...
    public function curriki_search_page_scripts() {
        ...
        wp_enqueue_style('qtip-css', plugins_url('/oer/assets/', realpath(__DIR__.'/../../')) . 'js/qtip2_v2.2.1/jquery.qtip.min.css', null, false, 'all'); // Add the styles first, in the <head> (last parameter false, true = bottom of page!)
        wp_enqueue_script('qtip-js', plugins_url('/oer/assets/', realpath(__DIR__.'/../../')) . 'js/qtip2_v2.2.1/jquery.qtip.min.js', array('jquery'), false, true); // Not using imagesLoaded? :( Okay... then this.
        //wp_enqueue_style('jquery-ui-css',  '//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css');
        wp_enqueue_script('jquery-ui-js',  '//code.jquery.com/ui/1.12.1/jquery-ui.js');
        // wp_enqueue_style('search-module-css', plugins_url('/', realpath(__DIR__)) . 'css/style.css', null, false, 'all');
        ...
        // wp_register_script('search-module-script', plugins_url('/js/script.js', realpath(__DIR__)), array('jquery'), false, true);
    ...
    public function curriki_search_page_body() {
        ...
        if (isset($this->request['phrase'])) { 
            $this->search_results_header_data = search_results_header_data();
        }
         
        $this->search_input_widget_data = search_input_widget_data();
                            
        if (isset($this->request['phrase'])) {
            
            if ($this->request['type'] == 'Resource') {
                $this->search_result_resources_data = search_result_resources_data();
            }
            
            if($this->partnerid!=3){
                
                if (isset($this->request['phrase']) && $this->status['found'] > $this->request['size']) {
                    $this->search_pagination_data = search_pagination_data();
                }
            }
        }
    }
                    ...
                    <div class="grid_3">
                        <div class="left-panel">
                            ...
                            <form action="<?php echo $this->search_page_url; ?>" method="GET" id="search_form" target="<?php echo $this->search_target; ?>">
                                ...
                                <?php 
                                    $this->search_input_widget_data = search_input_widget_data();
                                    // get_template_part('modules/search/views/search_input_widget'); 
                                ?>
                            </form>
                            </div>
                        </div>
                    ...
                    <div class="grid_9">
                        <div class="result-wrap">
                            <?php if (isset($this->request['phrase'])) { ?>
                                ...
                                <?php if ($this->request['type'] == 'Resource') {
                                        // get_template_part('modules/search/views/search_result_resources'); 
                                        $this->search_result_resources_data = search_result_resources_data();
                                    }
                                ?>
                                ...
                                <?php if($this->partnerid!=3){ ?>
                                    <?php 
                                        if (isset($this->request['phrase']) && $this->status['found'] > $this->request['size']) {
                                            // get_template_part('modules/search/views/pagination'); 
                                            $this->search_pagination_data = search_pagination_data();
                                        }
                                    ?>
                                <?php } ?>
